...
function getColumnHeaders(): string[] {
  return ['Client ID', 'Transaction ID', 'Activity Type', 'Details', 'Amount', 'Date'];
}
 ...
 */
export function createClientDebitsBalancesTab(transactions: Transaction[]): TabData {
  // Filter transactions by side = 'DEBIT'
  const creditTransactions = transactions.filter(transaction => transaction.side === 'DEBIT');
